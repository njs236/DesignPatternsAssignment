...
class Controller(FileReader, DataChecker, Singleton):
    ...
    def openFile(self, filename=None):
        if filename is None:
            filename = self.myViews['Console'].readLine("Enter a valid filename:")
...
class Model(Singleton):
    ...
    def check(self, reader):
        wrongLines = 0
        myChecker = CheckerMediator()
        try:
            for row in reader:
                wrong = myChecker.readLine(row)
                if (not wrong):
